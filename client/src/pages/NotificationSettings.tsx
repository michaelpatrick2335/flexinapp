...
const SECTIONS: { title: string; toggles: ToggleDef[] }[] = [
  {
    title: "ACTIVITY",
    toggles: [
      { key: "squadActivity",   label: "Squad activity",        description: "When teammates lift, PR, or hit Ghost Mode.", defaultValue: true },
      { key: "energyReceived",  label: "Energy received",       description: "When someone sends you a charge.",           defaultValue: true },
      { key: "weeklyMVP",       label: "Weekly MVP",            description: "Sundays when the new MVP is crowned.",       defaultValue: true },
    ],
  },
  {
    title: "REMINDERS",
    toggles: [
      { key: "workoutReminder", label: "Workout reminder",      description: "Daily nudge if you haven't logged a lift.",  defaultValue: true },
      { key: "weeklyScan",      label: "Weekly Scan",           description: "Sunday reminder to scan your week.",         defaultValue: true },
      { key: "streakRisk",      label: "Streak at risk",        description: "Heads-up before you break your streak.",     defaultValue: true },
    ],
  },
  {
    title: "COACH",
    toggles: [
      { key: "maxMessages",     label: "MAX coach messages",    description: "Tips and push from your AI coach.",          defaultValue: true },
      { key: "marketingEmail",  label: "Email digests",         description: "Weekly recap, product updates.",             defaultValue: false },
    ],
  },
];
...
export function NotificationSettings({ onBack }: Props) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: t.bg, color: t.text, paddingBottom: 60 }}>
      <Header t={t} title="Notification Settings" onBack={onBack} />
